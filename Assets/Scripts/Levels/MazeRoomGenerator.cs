...
{
    [Range(0f, 1f)]
    public float enemyOdds = 0.1f;
    private int maxEnemies = 3;

    public GameObject templateBrick;
    ...
    private static MazeRoomGenerator instance;
    public static MazeRoomGenerator Instance
    {
        get
        {
            return instance;
    ...
    void GenerateMaze()
    {
        ...
        int enemyTiles = 0;
        GameObject[,] currentSpawnedTiles = new GameObject[Maze.GetUpperBound(0), Maze.GetUpperBound(1)];
        for (int i = 1; i <= Maze.GetUpperBound(0) - 1; i++)
        {
            for (int j = 1; j <= Maze.GetUpperBound(1) - 1; j++)
            {
                if (Maze[i, j] == 1)
                {
                    if (j > 1 && (UnityEngine.Random.value > (1 - enemyOdds)) && enemyTiles < maxEnemies)
                    {
                        //if the enemy can stand somewhere
                        if (j < Maze.GetUpperBound(1) - 1 && Maze[i, j - 1] == 1 && currentSpawnedTiles[i, j - 1] == null)
                        {
                            ptype = GameObject.Instantiate(templateEnemy);
                            //remove the tiles at i+1 and i-1...
                            if (i > 1 && i < Maze.GetUpperBound(0))
                            {
                                if (currentSpawnedTiles[i - 1, j])
                                {
                                    GameObject.Destroy(currentSpawnedTiles[i - 1, j]);
                                }
                                if (currentSpawnedTiles[i + 1, j])
                                {
                                    GameObject.Destroy(currentSpawnedTiles[i - 1, j]);
                        ...
                        {
                            ptype = GameObject.Instantiate(templateBrick);
                        }
                        currentSpawnedTiles[i, j] = ptype;
                    }
                    else
                    {
                        ptype = GameObject.Instantiate(templateBrick);
                    ...
                    ptype.transform.position = transform.position + new Vector3(i * ptype.transform.localScale.x, j * ptype.transform.localScale.y, 0);
                }
                else if (Maze[i, j] == 0)
                {
